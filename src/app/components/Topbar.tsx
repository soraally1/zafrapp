...
export default function Topbar({ userName, userRole, userPhoto, loading = false }: TopbarProps) {
  ...
  return (
    <header className="sticky top-0 z-20 w-full bg-gradient-to-r from-[#E6FFF4]/80 via-white/80 to-[#F0FFF8]/80 backdrop-blur-lg border-b border-gray-100 flex items-center justify-between px-4 sm:px-8 py-4 md:py-5 shadow-xl transition-shadow">
      <div className="flex flex-col gap-1">
        ...
        {loading ? (
          ...
          <span className="font-extrabold text-2xl md:text-3xl text-gray-900 tracking-tight font-jakarta drop-shadow-sm flex items-center gap-2">
            {userName}
            {userRole && (
              <span className="ml-2 px-2 py-0.5 rounded-full bg-[#00C570]/10 text-[#00C570] text-xs font-semibold border border-[#00C570]/30 animate-fadeIn">
                {userRole}
              </span>
            )}
          </span>
        ...
        {/* Islamic Quote */}
        <div className="mt-2 bg-white/80 border-l-4 border-[#00C570] pl-3 pr-2 py-2 rounded-lg text-xs italic text-gray-700 max-w-xs shadow-md flex gap-2 items-start relative animate-fadeIn">
      ...
      <div className="flex items-center gap-4 relative">
        ...
        {/* Dropdown Menu */}
        {dropdownOpen && (
          <div className="absolute right-0 top-14 w-48 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-50 animate-fadeInDropdown origin-top-right">
            <button
              className="w-full text-left px-5 py-2 text-gray-700 hover:bg-[#E6FFF4] hover:text-[#00C570] font-medium rounded-t-2xl transition-colors flex items-center gap-2 duration-150"
            ...
            <button
              className="w-full text-left px-5 py-2 text-gray-700 hover:bg-red-50 hover:text-red-600 font-medium rounded-b-2xl transition-colors flex items-center gap-2 duration-150"
